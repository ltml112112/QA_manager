...
### 1. `06_lot_schedule` 캘린더 전체 재렌더 최소화
...
#### 2-1. CTQ/CTP 마스터 + 표준 데이터 모델 (가성비 최고)
...
#### 2-2. Firebase 누적 저장 (06번 앱 패턴 재사용)
...
- Firebase DB URL·API 키 평문 노출 — 정상. 보안은 Security Rules 로 담보. RTDB 규칙은 `@ltml.co.kr` 도메인 + 인증 필요로 설정 완료.
