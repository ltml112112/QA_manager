...
### 공정 Lot (Process Lot)
15번 P/N 공정 Flow에서 `pn_flow_docs/{docId}` 안의 한 Lot 단위. 합성·정제 공정의 1차 결과물로, `finalQty`(산출량)·`unit` 필드를 갖고 재고로 관리됨.

### 출하 Lot (Shipment Lot)
고객사로 출하되는 단위. `pn_flow_shipments/{shipId}`에 저장되며 `components[]`에 여러 **공정 Lot**(P/N/S 혼합·멀티 batch)을 N:M으로 묶을 수 있음. 컴포넌트 추가 시 출처 공정 Lot의 `finalQty`에서 자동 차감. 소프트 삭제(`deleted: true`) 지원 — 복원하면 재고도 같이 회복.

### 산출량 (finalQty)
공정 Lot의 마지막 공정 후 산출 중량. 단위는 `mg / g / kg`. **재고 = 산출량 − Σ(출하 components.qty)**. 미입력(`null`) 시 재고 배지 미표시. 15번 앱이 입력·차감 관리.
